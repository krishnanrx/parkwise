...
def draw_overlay(frame, detections, fps: float):
    ...
    cv2.putText(frame, f"FPS: {fps:.1f} | Plates: {len(detections)}", (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
    return frame
...
def write_logs(csv_path, jsonl_path, row):
    ...
    with open(csv_path, "a", newline="") as f:
        ...
        w.writerow([row["timestamp"], row["plate_text"], f"{row['confidence']:.3f}", *row["bbox"]])
...
def main():
    ...
    camera_type = cfg.get("io", {}).get("camera_type", "in")  # Default to "in" for single camera
            ...
            for det in detections:
                ...
                if det.get("plate_text", "").strip():
                    ...
                    if matched_data and det.get("valid", False):
                        ...
                        vehicle_number = matched_data.get('vehicle_number', det["plate_text"])
                        print(f"✅ Matched plate {vehicle_number} (ID: {matched_data.get('id')}) - updated session CSV")
    ...
    t_cap.start(); t_inf.start(); t_disp.start()
