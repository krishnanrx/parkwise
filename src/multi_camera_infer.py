"""
Multi-camera ANPR system with Firebase gate control
Supports entry and exit cameras for automatic gate control
"""
import argparse
import json
import os
...
from src.firebase_gate import control_gate, get_firebase_control
...
def draw_overlay(frame, detections, fps: float, camera_type: str = ""):
    ...
    status_text = f"FPS: {fps:.1f} | Plates: {len(detections)} | Camera: {camera_type.upper()}"
    cv2.putText(frame, status_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
    return frame
...
def write_logs(csv_path, jsonl_path, row):
    ...
    with open(csv_path, "a", newline="") as f:
        w = csv.writer(f)
        if write_header:
            w.writerow(["timestamp", "plate_text", "confidence", "x1", "y1", "x2", "y2", "camera_type", "matched"]) 
        w.writerow([
            row["timestamp"], 
            row["plate_text"], 
            f"{row['confidence']:.3f}", 
...
def process_camera(
    ...
    def infer_thread():
        fps = 0.0
        # DELAY CONFIGURATION: Change 2.0 to your desired delay in seconds (LINE 195)
        ...
        # Structure: {car_id: {'first_time': float, 'bbox': [x1,y1,x2,y2], 'ocr_done': bool, 'ocr_failed': bool, 'last_seen': float}}
        ...
        while not stop_flag.is_set():
            ...
            tracked_cars = {k: v for k, v in tracked_cars.items() if current_time - v['last_seen'] < 10.0}
            ...
            for b, s in zip(boxes, scores):
                ...
                if should_run_ocr:
                    ...
                    if ok and plate_text and permanent_parking_db:
                        matched_data = find_matching_plate(plate_text, permanent_parking_db, max_distance=2)
                        if matched_data:
                    ...
                    if time_since_detection < OCR_DELAY_SECONDS:
                        display_text = f"Waiting... ({OCR_DELAY_SECONDS - time_since_detection:.1f}s)"
                ...
                record = {
                    ...
                    "confidence": float(min(1.0, max(0.0, s if detector_only else (0.5 * s + 0.5 * plate_conf)))),
                    ...
                    if current_time - last_open > 10:  # 10 second cooldown per plate
                        ...
                        if success:
                            ...
                            if auto_close_delay > 0:
                                def close_gate_delayed():
                                    time.sleep(auto_close_delay)
                                    control_gate(vehicle_number, camera_type, False)
                                    print(f"🚪 Gate auto-closed after {auto_close_delay}s for {vehicle_number}")
                                threading.Thread(target=close_gate_delayed, daemon=True).start()
        ...
        while not stop_flag.is_set():
            ...
            for det in detections:
                if mode in ("json", "both"):
                    print(f"[{camera_name}] Detection: {det}")
...
def main():
    parser = argparse.ArgumentParser(description="Multi-camera ANPR system with Firebase gate control")
    parser.add_argument("--config", type=str, default=os.path.join(os.path.dirname(os.path.dirname(__file__)), "configs", "config.yaml"))
    ...
    permanent_parking_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "logs", "shobha_permanent_parking.csv")
    session_csv_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "logs", "shobha_permanent_parking_sessions.csv")
    ...
    if firebase_enabled:
        firebase_control = get_firebase_control()
        if firebase_control.is_connected():
            print("✅ Firebase connected - Gate control enabled")
        else:
            print("⚠️  Firebase not connected - Gate control disabled")
